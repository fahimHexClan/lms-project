  ...
  return (
    <PageLayout>
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h1 className="font-display text-3xl font-700 text-white">Lecturer Dashboard</h1>
          <p className="text-gray-500 mt-1">Overview of your modules and student activity.</p>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Students',   value: stats.students,    color: 'text-primary-400' },
            { label: 'Assignments',      value: stats.assignments,  color: 'text-blue-400' },
            { label: 'Submissions',      value: stats.submissions,  color: 'text-green-400' },
            { label: 'Awaiting Grade',   value: stats.ungraded,    color: stats.ungraded > 0 ? 'text-amber-400' : 'text-green-400' },
          ].map(s => (
        ...
        <div className="card">
          <h3 className="font-display text-white font-700 mb-4">Recent Submissions</h3>
          {recentSubmissions.length === 0 ? (
            <p className="text-gray-600 text-sm text-center py-6">No submissions yet</p>
          ) : (
              ...
              {recentSubmissions.map(s => (
                <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-800/50">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-300 truncate">{s.fileName}</p>
                    <p className="text-xs text-gray-500">{s.studentId?.slice(0, 8)}…</p>
                  </div>
                  ...
                  <a href="/lecturer/assignments" className="text-xs text-primary-400 hover:text-primary-300">Grade →</a>
                </div>
              ))}
            </div>
          )}
