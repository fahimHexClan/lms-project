  ...
  return (
    <div className="card">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-display text-white font-700">Leaderboard</h3>
      </div>
        ...
        {leaders.map((l) => (
          ...
          >
            <span className="text-lg w-7 text-center flex-shrink-0">
              {medals[l.rank - 1] || <span className="text-gray-500 text-sm font-mono">{l.rank}</span>}
            </span>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-200 truncate">
                ...
                {l.id === user?.uid && <span className="text-primary-400 ml-1">(you)</span>}
              </p>
              {l.badges?.length > 0 && (
                <p className="text-xs text-gray-500 truncate">{l.badges.slice(0, 3).join(' • ')}</p>
              )}
            </div>
            <span className="text-sm font-mono font-600 text-accent-400 flex-shrink-0">
