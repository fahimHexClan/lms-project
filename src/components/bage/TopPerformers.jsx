...
// Ranks students in the SAME batch + module by their combined academic
// performance — average % across BOTH graded assignments and exam results —
// not gamification points. Shows the top 5 in this cohort/course.
export default function TopPerformers() {
  ...
  useEffect(() => {
    ...
    const compute = async () => {
      ...
      // Graded assignment submissions belonging to students in this cohort
      const graded = subSnap.docs
        .map(d => d.data())
        .filter(s => studentIds.has(s.studentId) && s.grade !== null && s.grade !== undefined)

      // Exam results belonging to students in this cohort
      const examResults = examResultSnap.docs
        .map(d => d.data())
        .filter(r => studentIds.has(r.studentId))

      // Combined average % per student, across assignments AND exams
  ...
  return (
    <div className="card">
      ...
      <p className="text-xs text-slate-500 mb-3">
        Ranked by combined assignment + exam average · {profile?.batch || 'All'} · {profile?.module || 'All'}
      </p>
        ...
        {top.map((s, i) => (
          ...
          >
            <span className="text-lg w-7 text-center flex-shrink-0">
              {medals[i] || <span className="text-slate-400 text-sm font-mono">{i + 1}</span>}
            </span>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500">{s.gradedCount} graded item{s.gradedCount !== 1 ? 's' : ''}</p>
            </div>
